// Online zdroje – sekce Zdroje

window.SOURCES_ONLINE = [

  // Příběh Doroty Šandorové – moderní paralela
  {name: 'SeznamZprávy – Dorotu Š. zabil ošetřovatel (podcast)', url: 'https://www.seznamzpravy.cz/clanek/audio-podcast-5-59-dorotu-s-zabil-osetrovatel-jeji-pribeh-muze-pomoci-vsem-klientum-ustavu-223834'},
  {name: 'Noc důstojnosti – Příběh Doroty Šandorové', url: 'https://nocdustojnosti.cz/pribeh-doroty-sandorove/'},
  {name: 'Respekt – Kdo zabil Dorotu Š.? (2022, může být za paywallem)', url: 'https://www.respekt.cz/tydenik/2022/47/kdo-zabil-dorotu-s'},

  // Akce T4 – česky
  {name: 'Holocaust.cz – Eutanázie 1939–1941', url: 'https://www.holocaust.cz/dejiny/holocaust/historicky-kontext/eutanazie-1939---1941/'},
  {name: 'Medium Seznam – Zapomenuté oběti nacistické eutanázie', url: 'https://medium.seznam.cz/clanek/ceska-historie-zapomenute-obeti-nacisticky-program-eutanazie-znamenal-smrt-i-pro-stovky-obcanu-ceskoslovenska-119698'},
  {name: 'ČRo Plus – Pocta zapomenutým obětem nacistů (2014)', url: 'https://plus.rozhlas.cz/pocta-zapomenutym-obetem-nacistu-6570351'},
  {name: 'ČRo Plus – Akce T4 byla příprava na holocaust (historik Fedorovič)', url: 'https://plus.rozhlas.cz/akce-t4-byla-priprava-na-holocaust-rika-historik-fedorovic-7568480'},
  {name: 'Deník – Hitler zastavil eutanázii po kázání biskupa von Galena', url: 'https://www.denik.cz/ze_sveta/hitler-eutanazie.html'},
  {name: 'Muži v Česku – Smrtelná Akce T4', url: 'https://www.muzivcesku.cz/galerie/smrtelna-akce-t4-mirila-na-postizene-lidi-nevhodne-pro-nacisticke-nemecko/'},
  {name: 'ÚSTR – Paměť a dějiny: Akce T4 (PDF)', url: 'https://www.ustrcr.cz/data/pdf/pamet-dejiny/pad1502/056-066.pdf'},

  // Pamětní místa
  {name: 'Saské Švýcarsko – Památník Pirna-Sonnenstein', url: 'https://www.saechsische-schweiz.de/cz/vyletni-cile/pamatnik-pirna-sonnenstein'},
  {name: 'Elbe-Labe – Muzeum a památník Pirna-Sonnenstein', url: 'https://www.elbelabe.eu/cz/poi/m/museum-gedenksstaette-pirna-sonnenstein/'},

  // Osobnosti
  {name: 'The History Place – Kázání kardinála von Galena (1941, anglicky)', url: 'https://www.historyplace.com/speeches/galen.htm'},
  {name: 'STSG – Biografie Heidrich (PDF, německy)', url: 'https://www.stsg.de/cms/sites/default/files/upload/dokumente/pdf/1_biografie_heidrich.pdf'},
  {name: 'Wikipedia – Clemens August von Galen', url: 'https://cs.wikipedia.org/wiki/Clemens_August_von_Galen'},
  {name: 'Wikipedia – Philipp Bouhler (anglicky)', url: 'https://en.wikipedia.org/wiki/Philipp_Bouhler'},
  {name: 'Wikipedia – Karl Brandt', url: 'https://cs.wikipedia.org/wiki/Karl_Brandt'},
  {name: 'Wikipedia – Joseph Goebbels', url: 'https://cs.wikipedia.org/wiki/Joseph_Goebbels'},

  // Témata a pojmy
  {name: 'Wikipedia – Eutanázie', url: 'https://cs.wikipedia.org/wiki/Eutanazie'},
  {name: 'Wikipedia – Akce 14f13 (anglicky)', url: 'https://en.wikipedia.org/wiki/Action_14f13'},
  {name: 'Wikipedia – Vladimir Iljič Lenin', url: 'https://cs.wikipedia.org/wiki/Vladimir_Ilji%C4%8D_Lenin'},

  // Propaganda a film
  {name: 'USHMM – Propagandistické materiály programu eutanázie (anglicky)', url: 'https://encyclopedia.ushmm.org/content/en/gallery/propaganda-for-the-euthanasia-program'},
  {name: 'Dokina – Žaluji / Ich klage an (propagandistický film, 1941)', url: 'https://dokina.tiscali.cz/film/ich-klage-an-b428263'},

  // Lékařská etika
  {name: 'Lékařská fakulta UP – Hippokratova přísaha (PDF)', url: 'https://www.lf.upol.cz/fileadmin/userdata/LF/hippokratuv_dum/Hippokratova_prisaha.pdf'},
  {name: 'Holocaust Museum Houston – Lékařská etika a holocaust (anglicky)', url: 'https://hmh.org/event/medical-ethics-and-the-holocaust-how-healing-becomes-killing-eugenics-euthanasia-and-extermination/'},

  // Právo
  {name: 'EUR-Lex – Listina základních práv EU (PDF, česky)', url: 'https://eur-lex.europa.eu/LexUriServ/LexUriServ.do?uri=OJ:C:2010:083:0389:0403:CS:PDF'},
  {name: 'PSP – Listina základních práv a svobod ČR', url: 'https://www.psp.cz/docs/laws/listina.html'},

  // Ostatní
  {name: 'Masarykova univerzita – T. G. Masaryk', url: 'https://100.muni.cz/100-pribehu-muni/t-g-masaryk'},
];
